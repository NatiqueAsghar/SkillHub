    ...
    <div className="bg-bgd rounded-lg shadow-md p-6 text-center max-w-xs mx-auto">
      ...
      <div className="mt-4">
        <Link to={`/profile/${username}`}>
          <button className="px-4 py-2 bg-transparent border border-line text-tl rounded-md hover:bg-gr hover:text-txt transition">
            View Profile
          </button>
        </Link>
      </div>
      ...
      <div className="mt-4">
        <h6 className="text-sm font-medium text-txt">Skills</h6>
        <div className="flex flex-wrap justify-center gap-2 mt-2">
          {skills.map((skill, index) => (
            <div
              key={index}
              className="bg-tl text-txt px-2 py-1 rounded-full text-xs font-medium"
            >
              {skill}
            </div>
          ))}
