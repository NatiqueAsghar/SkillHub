    ...
    <div className="px-6 py-8">
      <div className="max-w-6xl mx-auto min-h-[86vh]">
        {loading ? (
          ...
          <>
            <div className="flex flex-col md:flex-row  border bg-bgd border-line shadow rounded-lg p-6">
                ...
                <div className="flex flex-col justify-center">
                  ...
                  {user?.username !== username && (
                    <div className="flex space-x-2 mt-2">
                      <button
                        className=" border border-line hover:bg-tl text-white px-4 py-2 rounded"
                        onClick={
                      ...
                      >
                        {connectLoading ? (
                          <div className="w-4 h-4 border-2 border-line border-t-transparent rounded-full animate-spin mr-2"></div>
                        ) : (
                          profileUser?.status
                        )}
                      </button>
                      <Link to={`/report/${profileUser.username}`}>
                        <button className=" border border-line bg-rd hover:bg-line text-txt px-4 py-2 rounded">
                          Report
                        </button>
                      </Link>
                      <Link to={`/rating/${profileUser.username}`}>
                        <button className=" border border-line bg-gr hover:bg-line text-txt px-4 py-2 rounded">
                          Rate
                        </button>
                      </Link>
                    </div>
              ...
              {user.username === username && (
                <div className="mt-4 md:mt-6 md:mr-2 md:ml-auto">
                  <Link to="/edit_profile">
                    <button className="text-txt">
                      <span className="hover:bg-tl">
              ...
              <div className="flex space-x-4 mt-4 md:mt-6">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="text-txt hover:text-teal-600 hover:scale-110 transition-all duration-200"
                >
                  <FaGithub />
                </a>
                <a
            ...
            <div className="mt-8 border-b border-line">
              ...
              <p className="text-txt text-sm">{profileUser?.bio}</p>
            </div>

            {/* Skills */}
            <div className="mt-8 border-b border-line flex flex-col items-center space-y-2">
              <h2 className="text-xl font-bold mb-4">Skills Proficient At</h2>
              <div className="flex flex-wrap gap-2">
                {profileUser?.skillsProficientAt.map((skill, index) => (
                  <div
                    key={index}
                    className="bg-bgd border border-line text-txt px-3 py-1 rounded-full text-sm"
                  >
                    {skill}
                  </div>
                ))}
            ...
            {profileUser?.projects?.length > 0 && (
              <div className="mt-8">
                <h2 className="text-xl text-txt font-bold mb-4">Projects</h2>
                <div className="space-y-4">
